...
(function(){
  ...
  function mood(tags){
    if(has(tags, ['social','pulse','local','island'])) return {
      line:'A few people are already moving around today.',
      search:'Island hopping, videoke, people nearby...',
      nudge:'Pulse is warm today.'
    };
    if(has(tags, ['quiet','private','slow','wellness'])) return {
      line:'Quiet beaches and slower corners are close by.',
      search:'Quiet beaches, hammocks, slow mornings...',
      nudge:'Look for calm spots.'
    };
    if(has(tags, ['food','market','recovery'])) return {
      line:'Fresh plates, market stops, and beach snacks are close by.',
      search:'Seafood, burgers, markets, coffee...',
      nudge:'Food is a good first stop.'
    };
    if(has(tags, ['explore','hidden','routes','surf','adventure'])) return {
      line:'Scooter roads, hidden places, and wild edges are calling.',
      search:'Waterfalls, scooter routes, hidden beaches...',
      nudge:'Explore is open.'
    };
    if(has(tags, ['photo','viewpoints','sunset'])) return {
      line:'The light should be worth chasing today.',
      search:'Viewpoints, sunsets, photo spots...',
      nudge:'Watch the light.'
    };
    if(has(tags, ['boat','curated'])) return {
      line:'Private boats and quiet water are worth checking today.',
      search:'Private boats, islands, quiet water...',
      nudge:'Start by the water.'
    };
    return {
      line:'The map is open. The day is still unwritten.',
      search:'Ask TALA or search San Vicente...',
      nudge:'Start anywhere.'
    };
  }
  ...
  function setHero(entry, m){
    ...
    if(title && name){
      title.innerHTML = greeting() + ', ' + esc(name) + '.';
    }
  ...
  function softOpenRelevantLayer(tags){
    ...
    // Do not steal the screen. Only make the matching bottom tab visually relevant.
    const tab = has(tags, ['social','pulse','local','island'])
      ? document.querySelector('.dock-item[data-tab="pulse"]')
      : document.querySelector('.dock-item[data-tab="discover"]');
    if(tab){
      tab.classList.add('vibe-ready');
      setTimeout(() => tab.classList.remove('vibe-ready'), 4200);
    }
  }
